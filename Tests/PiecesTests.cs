...
namespace myChess.Tests
{
    ...
    public class PiecesTests
    {
        ...
        [InlineData(typeof(Knight))]
        [InlineData(typeof(Pawn))]
        [InlineData(typeof(Queen))]
        [InlineData(typeof(Rook))]
        public void PieceCantBeOnTheSamePosition(Type type)
        {
            //Given
            IPiece piece = Activator.CreateInstance(type, Color.White) as IPiece;
            //When
            bool result = piece.CanMove(
                StartingColumn: Column.A,
                StartingRow: Row.Second,
                DestinationColumn: Column.A,
        ...
        public void PawnCanMoveDiagonallyIfCapturesAPiece()
        {
            ...
            //Given
            IPiece whitePawn = new Pawn(Color.White);
            IPiece blackPawn = new Pawn(Color.Black);

            IEnumerable<IHouse> housesList = new List<House>{
                new House(Column.A,Row.First){
                        PieceInLocation=whitePawn
                    },
                new House(Column.B,Row.Second){
                        PieceInLocation=blackPawn
                    }
            };

            //When
            bool result = whitePawn.CanMove(
                StartingColumn: Column.A,
